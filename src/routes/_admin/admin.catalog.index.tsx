...
import { Plus, Pencil, Trash2, Upload, FileText, ImageOff } from "lucide-react";
import { toast } from "sonner";
import {
  adminListCatalogItems,
  adminDeleteCatalogItem,
  adminSetCatalogItemPublished,
...
function Page() {
  const list = useServerFn(adminListCatalogItems);
  const del = useServerFn(adminDeleteCatalogItem);
  const setPublished = useServerFn(adminSetCatalogItemPublished);
  const router = useRouter();
  const queryClient = useQueryClient();
  const queryKey = ["admin-catalog-items"] as const;
  ...
  const toggle = useMutation({
    mutationFn: (vars: { id: string; is_published: boolean }) => setPublished({ data: vars }),
    onMutate: async (vars) => {
  ...
  async function handleDelete(id: string, title: string) {
    if (!confirm(`Удалить позицию "${title}"?`)) return;
    await del({ data: { id } });
    await refetch();
    router.invalidate();
  }
  ...
  return (
    <div className="p-8 max-w-6xl">
      ...
      <div className="flex flex-col items-center gap-3 mb-6 md:flex-row md:justify-between">
        <h1 className="font-serif text-3xl text-center md:text-left">Каталог</h1>
        <div className="flex flex-wrap justify-center gap-2">
          <Link to="/admin/catalog/page" className="inline-flex items-center gap-2 px-4 py-2 border border-border rounded-sm font-serif text-sm">
            <FileText className="w-4 h-4" /> Контент страницы
          </Link>
          <Link to="/admin/catalog/import" className="inline-flex items-center gap-2 px-4 py-2 border border-border rounded-sm font-serif text-sm">
            <Upload className="w-4 h-4" /> Импорт JSON
          </Link>
          <Link to="/admin/catalog/new" className="inline-flex items-center gap-2 px-4 py-2 bg-accent text-primary-foreground rounded-sm font-serif text-sm">
            <Plus className="w-4 h-4" /> Новая позиция
          </Link>
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead className="bg-secondary/40 text-left">
              <tr>
                ...
                <th className="px-4 py-3 font-serif">Категория</th>
                <th className="px-4 py-3 font-serif">Порядок</th>
            ...
            <tbody>
              {(data ?? []).length === 0 && (
                <tr><td colSpan={7} className="px-4 py-8 text-center text-muted-foreground">Пока нет позиций.</td></tr>
              )}
              {(data ?? []).map((d) => (
                <tr key={d.id} className="border-t border-border">
                  <td className="px-4 py-3">
                    {d.image_url ? (
                      <img src={d.image_url} alt="" className="w-12 h-12 object-cover rounded-sm border border-border" />
                    ) : (
                  ...
                  <td className="px-4 py-3">
                    <button
                      ...
                      aria-label={d.is_published ? "Скрыть позицию" : "Опубликовать позицию"}
                      title={d.is_published ? "Нажмите, чтобы скрыть" : "Нажмите, чтобы опубликовать"}
                      onClick={() => toggle.mutate({ id: d.id, is_published: !d.is_published })}
                  ...
                  <td className="px-4 py-3 flex gap-2">
                    <Link to="/admin/catalog/$id" params={{ id: d.id }} className="p-1.5 hover:bg-secondary rounded-sm">
